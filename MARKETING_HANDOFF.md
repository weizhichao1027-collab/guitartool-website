...
> 当前公开口径（最高优先级）：以 App Store 各地区实际可下载页面为准。公开营销不写版本号、Build 号或“已上线”；待审版本与后置闪光灯节拍等尚未公开功能全部隐藏。审核通过并确认商店公开后，才可统一恢复相关专题页和推广素材。下文带日期的旧版本记录只用于历史追溯，不得直接复制发布。
...
- 中英文技术支持页：`/support/` 与 `/en/support/`，覆盖当前公开版本的权限、调音器、节拍器、Widget、Apple Watch 和和弦卡片排障。
...
- App Store 当前公开可下载版本为 1.0.7；1.0.8 Build 8 正在等待审核，不得写成已上线。13 种语言支持网址与 7 张截图保持稳定。
- 主产品页当前使用 7 张截图，顺序为：节拍器、调音器、和弦库、和弦分享、四套主题、Widget、多设备。
- 网站、Press Kit、支持页与归档推广文案统一采用常青口径，只描述当前公开可用功能；待审功能隐藏。
...
- 三个自定产品页均有已批准版本。当前下载样本仍不足以支撑稳定显著性，因此先保持基线并积累 7–14 天数据，不立即创建 A/B 测试。
...
- App 内和弦卡片采用“预览 → 保存到相册 / 分享图片”的流程，系统分享只发送图片，不附带 App Store 链接，因此不使用 `app_chord_share` Campaign。卡片的品牌名、乐器、功能标签与高把位起始品位覆盖全部 13 种语言；中文品牌名为“吉他工具”，乐器标题为“吉他和弦指法”或“尤克里里和弦指法”，和弦名称居中放在和弦图正上方，高把位示例为中文“5 品”、英文“5fr”。该功能是当前公开版本的常青卖点。
...
6. 当前可推广和弦卡片分享与 Watch 后台声音；后置闪光灯节拍仍属待审内容，必须隐藏。Watch 必须说明仅继续声音，界面和触觉刷新会暂停，部分旧型号可能需要蓝牙音频路线。
...
- OpenAI Sites 当前 Version 17 已公开，但待审功能不得部署到 Sites。以后保存或公开部署新版本前必须重新取得明确确认；源码同步、版本保存和生产部署是三个不同状态。
...
3. 以 App Store 当前公开可下载版本为基线；不对外写版本号或 Build 号。13 个支持网址与 7 张截图保持稳定，App Preview 为 0。
...
10. 核对上级目录的 30 天推广看板、渠道总表、B 站文案与 13 语言社交文案；公开状态变化时四处同步，始终使用常青口径。
