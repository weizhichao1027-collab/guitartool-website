...
| Metronome Search Page | `ad230a27-b902-4645-b00d-3f2a5c3e3558` | `guitartool://metronome` | BPM、TAP、细分、拍号、三连音、Swing、渐进加速、速度训练 | 现有版本已批准；新版本等待审核 |
| Chord Library Search Page | `67d874cf-ac10-412d-929b-b883f2039444` | `guitartool://chords` | 指板、指法图、和弦库、查询、多种按法、和弦卡片、分享、相册 | 现有版本已批准；新版本等待审核 |

2026-08-28 已登录 App Store Connect 回读，三个页面全部进入“已批准”分组。它们均有 13 种语言推广文本，并分别把对应功能截图放到第一位；网站的 `ppid` 深链现已直接展示对应页面。当天 17:02 又为 Chord 与 Metronome 创建替代版本，更新简体中文、英语（美国）的推广文本和页面意图关键词，其他 11 种本地化与原截图继续继承，并在同一审核单中提交。App Store Connect 已确认两个项目均为“等待审核”；现有获批版本和公开链接在替代版本获批前继续有效。
...
- 1.0.8 正在等待审核；三个自定产品页的现有版本均已批准，Chord 与 Metronome 的替代版本也在等待审核。当前下载样本仍不足以支撑稳定显著性，因此本轮不创建 A/B 测试、不撤回任何审核提交。
...
4. 为已批准的 Chord 与 Metronome 自定产品页创建新版本前，先准备 13 语言完整改稿和关键词映射；发布会产生新的审核流程，不停用现有链接。
...
8. 获得明确发布确认后，再创建并发布 Chord 与 Metronome 自定产品页新版本；提交完成后不得创建重复精选提名。
